...
def render_alerts_module(df):
    ...
    critical_delay_sites = df[df['Schedule Variance (Days)'] > 14] if 'Schedule Variance (Days)' in df.columns else df.iloc[0:0]
    cost_overrun_sites = df[df['CPI'] < 0.95] if 'CPI' in df.columns else df.iloc[0:0]
    critical_risk_sites = df[df['Risk'] == 'Critical'] if 'Risk' in df.columns else df.iloc[0:0]
    ...
    with tab_webhook:
        ...
        if st.button("🚀 Dispatch Webhook Alert"):
            summary_msg = f"PROJECT PLUS ALERT: {len(critical_delay_sites)} critically delayed site(s) and {len(cost_overrun_sites)} cost overrun risk site(s) detected across Riyadh and regional hubs."
    ...
    if len(critical_delay_sites) > 0:
        for _, site in critical_delay_sites.iterrows():
            ...
            <div class="alert-box-critical">
                ...
                <span style="font-size:0.95rem;">Baseline Finish: {site['Baseline Finish'].strftime('%Y-%m-%d') if hasattr(site['Baseline Finish'], 'strftime') else site['Baseline Finish']} | Forecast Finish: {site['Forecast Finish'].strftime('%Y-%m-%d') if hasattr(site['Forecast Finish'], 'strftime') else site['Forecast Finish']}</span><br/>
